...
const SIGNER_PRIVATE_KEY = process.env.SIGNER_PRIVATE_KEY as
  | `0x${string}`
  | undefined;
const TREASURY_ADDRESS = process.env.TREASURY_ADDRESS as
  | `0x${string}`
  | undefined;
const FEE_VAULT_ADDRESS = process.env.FEE_VAULT_ADDRESS as
  | `0x${string}`
  | undefined;
...
// Tolerance for amount matching (0.5%) to handle minor wallet/bridge fee deductions
// e.g. user sends 0.1999... when expected is 0.2
const AMOUNT_TOLERANCE_BPS = 80n; // 50 basis points = 0.8%
...
async function batchDisburse(
  ...
  if (!TREASURY_ADDRESS || !FEE_VAULT_ADDRESS) {
    throw new Error(
      "TREASURY_ADDRESS and FEE_VAULT_ADDRESS are required for batch disbursement",
    );
  }

  const feeAmount =
    (totalAmount * BigInt(Math.round(FEE_RATE * 10000))) / 10000n;
  const netAmount = totalAmount - feeAmount;

  log(
    jobId,
    `Disbursing: net=${netAmount} → ${TREASURY_ADDRESS}, fee=${feeAmount} → ${FEE_VAULT_ADDRESS}`,
  );
...
async function processWatchDeposit(job: {
  id?: string;
  data: PendingTxJobData;
}) {
  ...
  log(
    jobId,
    `Starting watch — deposit=${depositId} wallet=${walletAddress} network=${network} token=${token} amount=${amount}`,
  );
  ...
  log(
    jobId,
    `Chain: ${network} (${chainConfig.chainId}), token contract: ${tokenAddress}`,
  );
  ...
  log(
    jobId,
    `Watching from block ${fromBlock} (current: ${startBlock}), token decimals: ${decimals}`,
  );
  ...
  while (Date.now() < deadline) {
    ...
    log(
      jobId,
      `Poll #${pollCount} — scanning blocks ${fromBlock}..${currentBlock}`,
    );
    ...
    try {
      ...
      if (logs.length > 0) {
        ...
        for (const transferLog of logs) {
          ...
          // Allow up to AMOUNT_TOLERANCE_BPS (0.5%) below the expected amount
          // e.g. 0.1999... qualifies for a 0.2 expected amount
          const minAcceptable =
            (expectedAmount * (10_000n - AMOUNT_TOLERANCE_BPS)) / 10_000n;
          if (transferValue >= minAcceptable) {
            log(
              jobId,
              `Amount matched (expected >= ${formatUnits(expectedAmount, decimals)}). Confirming payment...`,
            );
            ...
            if (SIGNER_PRIVATE_KEY && TREASURY_ADDRESS && FEE_VAULT_ADDRESS) {
              try {
                ...
                log(
                  jobId,
                  `Disbursement signer=${account.address}, treasury=${TREASURY_ADDRESS}, feeVault=${FEE_VAULT_ADDRESS}`,
                );
                ...
                logError(
                  jobId,
                  `Batch disbursement failed (deposit already confirmed):`,
                  disburseError,
                );
              ...
              log(
                jobId,
                `Skipping batch disbursement — SIGNER_PRIVATE_KEY, TREASURY_ADDRESS, or FEE_VAULT_ADDRESS not set`,
              );
              const result = {
                settled: true,
                depositId,
  ...
  log(
    jobId,
    `Watch timed out after ${WATCH_TIMEOUT_MS / 1000}s — no matching transfer found`,
  );
  ...
  throw new Error(
    `WATCH_TIMEOUT: No matching ${token} transfer to ${walletAddress} on ${network} within ${WATCH_TIMEOUT_MS / 1000}s`,
  );
}
...
log(
  undefined,
  `Worker started — queue="${PENDING_TX_QUEUE}", timeout=${WATCH_TIMEOUT_MS / 1000}s, poll=${POLL_INTERVAL_MS / 1000}s`,
);
log(
  undefined,
  `Batch disbursement: ${SIGNER_PRIVATE_KEY ? "ENABLED" : "DISABLED (no SIGNER_PRIVATE_KEY)"}`,
);
