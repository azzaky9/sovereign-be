...
const sepolia = defineChain({
  ...
  rpcUrls: {
    default: {
      http: [process.env.ETH_SEPOLIA_RPC_URL ?? "https://rpc.sepolia.org"],
    },
  },
  blockExplorers: {
...
const opSepolia = defineChain({
  ...
  rpcUrls: {
    default: {
      http: [process.env.OP_SEPOLIA_RPC_URL ?? "https://sepolia.optimism.io"],
    },
  },
  blockExplorers: {
    default: {
...
export function getChainConfig(networkKey: string): ChainEntry {
  ...
  console.log("using network", { entry, networkKey });
  if (!entry) {
    throw new Error(
